...
The database dump is represented as an immutable DB SourceSnapshot:
...
The source inventory was checked directly against an immutable CODE SourceSnapshot.
...
For CODE, line ranges remain navigation hints only. The logical anchor is:
...
# 3.1. Source Snapshots

RP-01 uses two immutable physical source snapshots.
...
These snapshots are immutable research inputs.

If the backend changes before Git is introduced:
...
is created as a new SourceSnapshot. Existing SourceFacts are not rewritten.

When the backend later moves to Git, a Git commit can become the artifact identity of a subsequent SourceSnapshot without changing the prototype model.
...
symbol
structural_anchor
navigation_hint (optional line range)
```
...
Provenance: PARTIALLY COMPLETE — concrete SourceFacts with SourceSnapshot anchors exist for the claims in §4.1
